...
import KelolaVideo from "./dashboardAdmin/KelolaVideo";
...
const AdminDashboard = () => {
   ...
   const [isModalInsertKontenOpen, setIsModalInsertKontenOpen] =
      useState(false);
   ...
   const handleDeleteKonten = (id) => {
      if (activeTab === "edukasi") {
         ...
         deleteVideoById(id)
            .then(() => {
               setListVideo((prev) =>
                  prev.filter((item) => item.idvideo !== id)
               );
   ...
   const handleUpdateArtikel = async (updatedArticle) => {
      try {
         ...
         setListArtikel((prev) =>
            prev.map((item) =>
               item.idartikel === updatedArticle.idartikel
                  ? updatedArticle
                  : item
            )
         );
   ...
   const handleEditKonten = (id) => {
      ...
      if (activeTab === "edukasi") {
         ...
         const kontenToEdit = listVideo.find((item) => item.idvideo === id);
         setDataToEdit(kontenToEdit);
      }
      ...
      try {
         if (activeTab === "edukasi") {
            ...
            setListWhiteNoise((prev) => {
               ...
               console.log(
                  "Updated listWhiteNoise state (AdminDashboard):",
                  newState
               ); // Tambahkan log ini
               return newState;
            ...
            console.log("data", data);
            const insertedVideo = await insertVideo(data); // Capture the returned data with ID
            setListVideo((prev) => [...prev, insertedVideo]); // Use the inserted data
            setIsModalInsertKontenOpen(false);
   ...
   const onEditKonten = async (data) => {
      try {
         if (activeTab === "edukasi") {
            ...
            setListEdukasi((prev) =>
               prev.map((item) =>
                  item.idedukasi === data.idedukasi ? data : item
               )
            );
         } else if (activeTab === "white-noise") {
            ...
            setListArtikel((prev) =>
               prev.map((item) =>
                  item.idartikel === data.idartikel ? data : item
               )
            );
         } else if (activeTab === "video") {
            await editVideo(data.idvideo, data);
            setListVideo((prev) =>
               prev.map((item) =>
                  item.idvideo === data.idvideo ? data : item
               )
            );
          }
   ...
   useEffect(() => {
    ...
    useEffect(() => {
      if (activeTab !== "video" || isVideoLoaded) return;
      let isMounted = true;
         ...
         >
            ...
            <nav className="flex-grow">
               <ul>
                  ...
                  <li className={`mb-4 rounded-lg transition duration-200 ${
                        activeTab === "video"
                           ? "bg-blue-600 text-white"
                           : "hover:bg-gray-700 text-gray-300"
                     }`}
                      ...
                      >
                        ...
                        >
                           <path  
                              ...
                              strokeLinejoin="round"
                              strokeWidth="2"
                              d="M15 10l4.5 3.5a1 1 0 010 1.5l-4.5 3.5A1 1 0 0114 18V6a1 1 0 011-1h6a1 1 0 011 1v12a1 1 0 01-1 1h-6a1 1 0 01-1-1v-4z"
                            ></path>
                        </svg>
                        Video
                      </button>
         ...
         <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
                  ...
                  <div className="container mx-auto">
                     ...
                     <div className="bg-white rounded-lg shadow-md p-6">
                        <p className="text-gray-700">
                           Selamat datang di halaman admin. Gunakan menu di
               ...
               {activeTab === "video" && (
                  <KelolaVideo
                     videos={listVideo}
                     onAddVideo={handleAddKonten}
                     onEditVideo={handleEditKonten}
                     onDeleteVideo={handleDeleteKonten}
                  />
               )}
               {isModalArtikelOpen && (
                  <ModalInsertArtikel
